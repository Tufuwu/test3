...
class CSSVariablesDeclaration(css_parser.util._NewBase):
    ...
    def __repr__(self):
        return "css_parser.css.%s(cssText=%r)" % (self.__class__.__name__, self.cssText)
    ...
    def __iter__(self):
        ...
        for name in as_list(self.keys()):
            yield name

    def keys(self):
        """Analoguous to standard dict returns variable names which are set in
        this declaration."""
        return as_list(self._vars.keys())

    def _getCssText(self):
        """Return serialized property cssText."""
        return css_parser.ser.do_css_CSSVariablesDeclaration(self)
