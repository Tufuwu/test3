...
class CSS2Properties(object):
    ...
    css_parser actually also allows usage of ``del`` to remove a CSS property
...
for group in css_parser.profiles.properties:
    for name in css_parser.profiles.properties[group]:
