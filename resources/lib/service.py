...
class AutoUpdater:
    ...
    def __init__(self):
        ...
        self.monitor = UpdateMonitor(
            update_settings=self.createSchedules,
            after_scan=self.databaseUpdated)
        self.readLastRun()
        ...
        if (not self.lock):
            ...
            while count < len(self.schedules):
                ...
                if (cronJob.next_run <= now):
                    if (not player.isPlaying() or utils.getSetting(
                            "run_during_playback") == "true"):

                        # check if run on idle is checked and screen is idle -
                        # disable this on manual run
                        if (not utils.getSettingBool('run_on_idle') or (utils.getSettingBool(
                                'run_on_idle') and (self.monitor.screensaver_running or manual))):

                            # check for valid network connection - check
                            # sources if setting enabled
                            if (self._networkUp() and (not utils.getSettingBool('check_sources') or (
                                    utils.getSettingBool('check_sources') and self._checkSources(cronJob)))):

                                # check if this scan was delayed due to
                                # playback
                                if (cronJob.on_delay):
                                    ...
                                    utils.log(
                                        cronJob.name + " paused due to playback")
    ...
    def createSchedules(self, forceUpdate=False):
        ...
        showDialogs = utils.getSettingBool('notify_next_run')

        if (utils.getSettingBool('clean_libraries')):
            # create clean schedule (if needed)
            if (utils.getSettingInt("clean_timer") != 0):

                if (utils.getSettingInt('library_to_clean') ==
                        0 or utils.getSettingInt('library_to_clean') == 1):
                    ...
                    aSchedule.command = {
                        'method': 'VideoLibrary.Clean', 'params': {
                    ...
                    if (utils.getSettingInt("clean_timer") == 4):
                        ...
                        aSchedule.expression = "0 0 " + \
                            aSchedule.cleanLibrarySchedule(utils.getSettingInt("clean_timer"))
        ...
        if (utils.getSettingBool('update_video')):
            ...
            aSchedule.next_run = self.calcNextRun(
                aSchedule.expression, self.last_run)
            self.schedules.append(aSchedule)
        ...
        utils.log("Created " +
                  str(len(self.schedules)) +
                  " schedules", xbmc.LOGDEBUG)
    ...
    def checkTimer(self, settingName):
        ...
        if (utils.getSettingBool(settingName + '_advanced_timer')):
            ...
            result = '0 */' + \
                str(utils.getSetting(settingName + "_timer")) + ' * * *'
    ...
    def showNotify(self, displayToScreen=True):
        ...
        for cronJob in self.schedules:
            if (cronJob.next_run <
                    next_run_time.next_run or next_run_time.next_run == 0):
                next_run_time = cronJob

        inWords = self.nextRunCountdown(next_run_time.next_run)
        # show the notification (if applicable)
        if (next_run_time.next_run > time.time() and utils.getSettingBool(
                'notify_next_run') and displayToScreen):
    ...
    def readLastRun(self):
        if (self.last_run == 0):
            ...
            if (xbmcvfs.exists(xbmcvfs.translatePath(
                    utils.data_dir() + "last_run.txt"))):
                ...
                try:
                    # there may be an issue with this file, we'll get it the
                    # next time through
        ...
        if (utils.getSettingBool('clean_libraries')):
            ...
            if (not xbmc.Player().isPlaying()
                ...
                if (utils.getSettingInt("clean_timer") == 0):
                    ...
                    if ((utils.getSettingInt('library_to_clean') == 0 or utils.getSettingInt(
                            'library_to_clean') == 1) and database == 'video'):
                        # create the clean job schedule
                        aJob.command = {
                            'method': 'VideoLibrary.Clean', 'params': {
                                'showdialogs': showDialogs}}
                    if ((utils.getSettingInt('library_to_clean') == 2 or utils.getSettingInt(
                            'library_to_clean') == 0) and database == 'music'):
                        aJob.command = {
                            'method': 'AudioLibrary.Clean', 'params': {
                                'showdialogs': showDialogs}}
    ...
    def _checkSources(self, aJob):
        ...
        if (aJob.command['method'] ==
                'VideoLibrary.Scan' or aJob.command['method'] == 'AudioLibrary.Scan'):
            ...
            if ('directory' in aJob.command['params']):
                ...
                result = self._sourceExists(
                    aJob.command['params']['directory'])
            else:
