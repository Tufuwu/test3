...
class ApiV2TestCase(TestCase):
    ...
    def _side_effect_do_request(*args):
        if args[0] == "/tracks":
            if args[1].get("ids") == "53787294":
                with open(
                    "./tests/mocks/api_v2_playlist_tracks.json"
                ) as f:
                    mock_data = f.read()
            else:
                with open(
                    "./tests/mocks/api_v2_discover_tracks.json"
                ) as f:
                    mock_data = f.read()
    ...
    def _side_effect_request_get(*args, **keywargs):
        if args[0] == "https://soundcloud.com/":
            ...
            args[0]
            == "https://a-v2.sndcdn.com/assets/0-744ba03a-3.js"
        ):
            with open(
                "./tests/mocks/html/assets.0-744ba03a-3.js"
            ) as f:
                mock_data = f.read()
            obj = mock.Mock()
            obj.text = mock_data
            ...
            args[0]
            == "https://a-v2.sndcdn.com/assets/49-4786eb1d-3.js"
        ):
            with open(
                "./tests/mocks/html/assets.49-4786eb1d-3.js"
            ) as f:
                mock_data = f.read()
            obj = mock.Mock()
            obj.text = mock_data
    ...
    def test_search(self):
        ...
        self.assertEqual(
            res.items[0].info["description"], "The description (1)"
        )
        self.assertEqual(res.items[0].info["duration"], 194.763)
        self.assertEqual(res.items[0].info["genre"], "Dubstep")
        self.assertEqual(
            res.items[0].info["date"], "2011-05-23T16:22:06Z"
        )
        self.assertEqual(
            res.items[0].media,
            "https://api-v2.soundcloud.com/media/soundcloud:tracks:15784497/580ad806-b3ab-440f-adbe-c12a83258a37/stream/progressive",
        )
        self.assertEqual(
            res.items[0].thumb,
            "https://i1.sndcdn.com/artworks-000007527658-smjpzh-t500x500.jpg",
        )
        ...
        self.assertEqual(
            res.items[1].info["description"], "The description (2)"
        )
        self.assertEqual(res.items[1].info["duration"], 389.371)
        self.assertEqual(res.items[1].info["genre"], "Dubstep")
        self.assertEqual(
            res.items[1].info["date"], "2011-09-17T15:39:49Z"
        )
        self.assertEqual(
            res.items[1].media,
            "https://api-v2.soundcloud.com/media/soundcloud:tracks:23547065/e7846551-5c8e-4b93-b4f0-f94bfa7b1275/stream/progressive",
        )
        self.assertEqual(
            res.items[1].thumb,
            "https://i1.sndcdn.com/artworks-000011681052-n1a6w6-t500x500.jpg",
        )

    def test_search_playlists(self):
        with open(
            "./tests/mocks/api_v2_search_playlists_without_albums.json"
        ) as f:
            mock_data = f.read()
        ...
        self.assertEqual(
            res.items[0].info["artist"], "Sebastian Morad"
        )
        self.assertEqual(
            res.items[0].thumb,
            "https://i1.sndcdn.com/artworks-000498621510-fk1ovg-t500x500.jpg",
        )
    ...
    def test_search_users(self):
        ...
        self.assertEqual(
            res.items[0].thumb,
            "https://i1.sndcdn.com/avatars-000451809714-n5njwk-t500x500.jpg",
        )
        ...
        self.assertEqual(
            res.items[1].thumb,
            "https://i1.sndcdn.com/avatars-000559848966-7tof1c-t500x500.jpg",
        )
    ...
    def test_playlist(self):
        ...
        self.api._do_request.side_effect = (
            self._side_effect_do_request
        )
        ...
        self.assertEqual(
            res.items[0].label,
            "Rock Your Body - Justin Timberlake (Alex Dogmatic Remix)",
        )
    ...
    def test_resolve_id(self):
        ...
        self.assertEqual(
            res.items[0].media,
            "https://api-v2.soundcloud.com/media/soundcloud:tracks:273627408/d35bd07a-3adb-4620-a876-7770f80ff48d/stream/progressive",
        )
    ...
    def test_resolve_url(self):
        ...
        res = self.api.resolve_url(
            "https://m.soundcloud.com/user/foo"
        )
        ...
        self.api._do_request.assert_called_with(
            ANY, {"url": "https://soundcloud.com/user/foo"}
        )
        self.assertEqual(
            res.items[0].label, "Thomas Hayden - Universe"
        )
        self.assertEqual(
            res.items[0].media,
            "https://api-v2.soundcloud.com/media/soundcloud:tracks:584959245/631cc995-e8f2-4a62-a212-5a5768046bc2/stream/progressive",
        )
    ...
    def test_discover(self):
        ...
        self.assertEqual(
            res.items[0].label, "Top 50: All music genres"
        )
        self.assertEqual(
            res.items[1].label, "Top 50: Alternative Rock"
        )
        self.assertEqual(res.items[2].label, "Top 50: Ambient")

        # Level 3
        res = self.api.discover(
            "soundcloud:system-playlists:charts-top:all-music:at"
        )
        self.assertEqual(res.load[0], 683327426)
        self.assertEqual(res.load[1], 591031647)
        self.assertEqual(res.items[0].label, "110")
        self.assertEqual(
            res.items[1].label, "THis iS thE LiFe - TEKK Remix"
        )
    ...
    def test_charts(self):
        ...
        self.assertEqual(
            res.items[1].label,
            "Young Nudy X Playboi Carti - Pissy Pamper Aka KID CUDI (Slimerre Shit)",
        )
    ...
    def test_audio_format(self):
        ...
        self.assertEqual(
            res.items[0].media,
            "https://api-v2.soundcloud.com/media/soundcloud:tracks:273627408/23d4e278-f8c0-4438-ace8-201dbd242a1c/stream/hls",
        )
    ...
    def test_fetch_client_id(self, mock_method):
        ...
        self.assertEqual(
            client_id, "1XduoqV99lROqCMpijtDo5WnJmpaLuYm"
        )
