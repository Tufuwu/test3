...
class IntTypeSpec(TypeSpec):
    ...
    def str_to_val(self, errors, pos, string, _module):
        ...
        try:
            ...
            err_add(
                errors, pos, "TYPE_VALUE", (string, self.definition, "not an integer")
            )
            return None
...
class Decimal64TypeSpec(TypeSpec):
    ...
    def str_to_val(self, errors, pos, string, _module):
        ...
        if syntax.re_decimal.search(string) is None:
            err_add(
                errors, pos, "TYPE_VALUE", (string, self.definition, "not a decimal")
            )
            return None
        if string[0] == "-":
            is_negative = True
...
class BooleanTypeSpec(TypeSpec):
    ...
    def str_to_val(self, errors, pos, string, _module):
        if string == "true":
            ...
            err_add(
                errors, pos, "TYPE_VALUE", (string, self.definition, "not a boolean")
            )
            return None
...
class BinaryTypeSpec(TypeSpec):
    ...
    def str_to_val(self, errors, pos, string, _module):
        try:
            ...
            err_add(errors, pos, "TYPE_VALUE", (string, "", "bad base64 value"))
...
def validate_range_expr(errors, stmt, type_):
    ...
    ranges = [
        (convert(m[1]), convert(m[6])) for m in syntax.re_range_part.findall(stmt.arg)
    ]
...
def validate_ranges(errors, pos, ranges, type_):
    ...
    for lo, hi in ranges:
        if isinstance(type_.i_type_spec, RangeTypeSpec):
            type_.i_type_spec.validate(errors, pos, (lo, hi), type_.i_module, "")
        else:
            if lo is not None and lo != "min" and lo != "max":
                type_.i_type_spec.validate(errors, pos, lo, type_.i_module, "")
            if hi is not None and hi != "min" and hi != "max":
                type_.i_type_spec.validate(errors, pos, hi, type_.i_module, "")
...
class RangeTypeSpec(TypeSpec):
    ...
    def validate(self, errors, pos, val, module, errstr=""):
        def inner_validate(errors, pos, val, module, errstr):
            ...
            for lo, hi in self.ranges:
                ...
                if (lo == "min" or lo == "max" or val >= cur_lo) and (
                    (hi is None and val == cur_lo)
                    or hi == "max"
                    or (hi is not None and val <= cur_hi)
                ):
                    return True, (lo, hi)
            err_add(
                errors,
        ...
        if isinstance(val, tuple):
            common_restriction(
                errors, pos, val, module, self, "range", inner_validate, errstr
            )
        else:
...
def validate_length_expr(errors, stmt, type_stmt):
    def f(lostr, histr):
        try:
            ...
            err_add(errors, stmt.pos, "TYPE_VALUE", (lostr, "", "not an integer"))
            return (None, None)
        try:
            if histr == "":
            ...
            err_add(errors, stmt.pos, "TYPE_VALUE", (histr, "", "not an integer"))
            return None
        return (lo, hi)
    ...
    for lo, hi in lengths:
        ...
        if (
            ...
            return None
        if lo == "min" and hi is not None and hi != "max" and hi < length_typespec.min:
...
class LengthTypeSpec(TypeSpec):
    ...
    def validate(self, errors, pos, val, module, errstr=""):
        def inner_validate(errors, pos, val, module, errstr):
            ...
            if isinstance(val, util.int_types):
                ...
                if (
                    ...
                    and cur_base.validate(errors, pos, val, module, errstr) is False
                ):
                    return False, None
                vallen = val
            else:
            ...
            for lo, hi in self.lengths:
                ...
                if (lo == "min" or lo == "max" or vallen >= cur_lo) and (
                    (hi is None and vallen == cur_lo)
                    or hi == "max"
                    or (hi is not None and vallen <= cur_hi)
                ):
                    return True, (lo, hi)
            err_add(
                errors,
        ...
        if isinstance(val, tuple):
            common_restriction(
                errors, pos, val, module, self, "length", inner_validate, errstr
            )
        else:
            res, ranges = inner_validate(errors, pos, val, module, errstr)
            return res
    ...
    def validate(self, errors, pos, val, module, errstr=""):
        ...
        for pattern in self.res:
            if pattern(val) is False:
                msg = "pattern mismatch {errstr} for pattern defined at {pos}".format(
                    errstr=errstr, pos=pattern.pos
                )
        ...
        if value is not None:
            try:
                ...
                if oldval is not None and oldval != x:
                    err_add(errors, value.pos, "BAD_ENUM_VALUE", (value.arg, oldval))
                e.i_value = x
                ...
                if x >= auto:
                    auto = x + 1
                if x in values:
                    err_add(errors, value.pos, "DUPLICATE_ENUM_VALUE", (x, values[x]))
                else:
                    values[x] = value.pos
        ...
        if position is not None:
            try:
                ...
                if oldpos is not None and oldpos != x:
                    err_add(
                        errors, position.pos, "BAD_BIT_POSITION", (position.arg, oldpos)
                    )
                b.i_position = x
                ...
                if x in values:
                    err_add(
                        errors, position.pos, "DUPLICATE_BIT_POSITION", (x, values[x])
                    )
                else:
    ...
    def parse_keypath(s):
        ...
        def parse_identifier(s):
            ...
            if m.group(2) is None:
                ...
                prefix = m.group(2)
                mod = util.prefix_to_module(path.i_module, prefix, path.pos, errors)
                if mod is not None:
...
class PathTypeSpec(TypeSpec):
    ...
    def str_to_val(self, errors, pos, string, module):
        if hasattr(self, "i_target_node"):
            return self.i_target_node.search_one("type").i_type_spec.str_to_val(
                errors, pos, string, module
            )
        else:
    ...
    def validate(self, errors, pos, val, module, errstr=""):
        if hasattr(self, "i_target_node"):
            return self.i_target_node.search_one("type").i_type_spec.validate(
                errors, pos, val, module, errstr
            )
        else:
            # if a default value is verified
            return True
