    ...
    def add_opts(self, optparser):
        optlist = [
            ...
            EditOption(
                ...
                dest="edit_delete_import_dates",
                default=False,
                action="store_true",
                help="Delete import/include revision-date " "statements",
            ),
            # set meta info (these do nothing if there's no corresponding
            # metadata statement)
            EditOption(
                "--edit-organization",
                dest="edit_organization",
                metavar="ORGANIZATION",
                help="Set module/submodule organization " "to the supplied value",
            ),
            EditOption(
                "--edit-contact",
                dest="edit_contact",
                metavar="CONTACT",
                help="Set module/submodule contact " "to the supplied value",
            ),
            EditOption(
                "--edit-description",
                dest="edit_description",
                metavar="DESCRIPTION",
                help="Set module/submodule description " "to the supplied value",
            ),
            # set revision info (these do nothing if there's no revision
            # statement)
            EditOption(
                "--edit-delete-revisions-after",
                dest="edit_delete_revisions_after",
                type="date",
                metavar="PREVDATE",
                help="Delete any revisions after " "the supplied yyyy-mm-dd",
            ),
            EditOption(
                "--edit-revision-date",
                dest="edit_revision_date",
                type="date",
                metavar="DATE",
                help="Set most recent revision date " "to the supplied yyyy-mm-dd",
            ),
            EditOption(
                "--edit-revision-description",
                dest="edit_revision_description",
                metavar="DESCRIPTION",
                help="Set most recent revision description " "to the supplied value",
            ),
            EditOption(
                "--edit-revision-reference",
                dest="edit_revision_reference",
                metavar="REFERENCE",
                help="Set most recent revision reference " "to the supplied value",
            ),
        ]
    ...
    for module in modules:
        ...
        for stmt in module.substmts:
            ...
            if stmt.keyword in ["import", "include"]:
                ...
                replstmts, revision_done = set_revision_details(ctx, stmt, lastrev)
...
def set_revision_details(ctx, stmt, lastrev):
    ...
    #   the new and the original statement
        ...
        if action == "replace":
            ...
            revstmt = statements.new_statement(
                stmt.top, stmt.parent, None, "revision", newdate
            )
...
def update_or_add_stmt(stmt, keyword, arg, index=None):
    ...
    if argval is None:
        ...
        if not replace and child.arg and child.arg != argval and child.arg != "TBD":
            sys.stderr.write(
                "%s: not replacing existing %s %r with %r\n"
