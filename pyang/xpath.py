...
yang_xpath_functions = {"current": ([], "node-set")}
        ...
        if q[0] == "absolute":
            ...
            if s[0] == s[-1] and s[0] in ("'", '"'):
                ...
                if i != -1 and s.find(":", i + 1) == -1:
                    ...
                    tag = s[i + 1 :]
                    if (
                        re_identifier.search(prefix) is not None
                        and re_identifier.search(tag) is not None
                    ):
                        ...
                        for pos0, code, arg in my_errors:
                            if code == "PREFIX_NOT_DEFINED" and t == "qstring":
                                ...
                                err_add(ctx.errors, pos0, "PREFIX_NOT_DEFINED", arg)
                            else:
                                # this may or may not be an error;
                                # report a warning
                                err_add(ctx.errors, pos0, "WPREFIX_NOT_DEFINED", arg)
...
def chk_xpath_function(ctx, mod, pos, initial, node, func, args):
    ...
    if nexp == 0:
        if nargs != 0:
            err_add(ctx.errors, pos, "XPATH_FUNC_ARGS", (func, nexp, nargs))
    elif signature[0][-1] == "?":
        ...
        if axis == "self":
            ...
            if pmodule is not None and node is not None and initial is not None:
                ...
                if child is None and node == "root":
                    ...
                    getattr(initial, "i_config", None) is True
