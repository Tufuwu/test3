    ...
    def add_opts(self, optparser):
        optlist = [
            optparse.make_option(
                "--omni-path", dest="omni_tree_path", help="Subtree to print"
            ),
        ]
...
def print_class_header(s, fd, ctx, root="false"):
    ...
    fd.write(
        'make new shape at end of graphics with properties {autosizing: full, size: {187.500000, 14.000000}, text: {{alignment: center, font: "Helvetica-Bold", text: "%s "}, {alignment: center, color:%s, font: "Helvetica-Bold", text: "%s "}}, text placement: top, origin: {150.000000, 11.500000}, vertical padding: 0}\n'
        ...
        for ch in s.i_children:
            ...
            if ch.keyword in ["leaf", "leaf-list"]:
                if not found_attrs:
                    ...
                    fd.write(
                        "make new shape at end of graphics with properties {autosizing:full, size:{187.5, 28.0}, text:{"
                    )
                    found_attrs = True
                else:
        ...
        for ch in s.i_children:
            if ch.keyword == ("tailf-common", "action"):
                if not found_actions:
                    fd.write(
                        'make new shape at end of graphics with properties {autosizing:full, size:{187.5, 28.0}, text:{text:"'
                    )
                    found_actions = True
...
def close_class(number, s, fd, ctx):
    fd.write("local %s\n" % fullpath(s))
    fd.write(
        "set %s to assemble ( graphics -%s through -1 ) table shape {%s, 1}\n"
...
def print_aggregation(parent, this, fd, lower, upper, ctx):
    fd.write(
        "connect %s to %s with properties {%s} \n"
...
def print_notification(notification, fd, ctx, root="false"):
    fd.write(
        "<UML:Class xmi.id = '%s' name = '%s' "
        % (fullpath(notification), notification.arg)
    )


def print_inout(parent, s, fd, ctx, root="false"):
    fd.write(
        "<UML:Class xmi.id = '%s' name = '%s-%s' "
        % (fullpath(s), parent.arg, s.keyword)
    )
...
def print_leaf(leaf, append, index, fd, ctx):
    ...
    if not index:
        fd.write(
            '{font: "Helvetica-Oblique", color: %s, text: "%s%s%s %s %s\n"}'
            % (color, leaf.arg, append, mand, c, get_typename(leaf))
        )
    else:
        fd.write(
            '{font: "Helvetica-Oblique", color: %s, underlined: true, text: "%s%s%s %s %s\n"}'
            % (color, leaf.arg, append, mand, c, get_typename(leaf))
        )
...
def print_text(t, fd, ctx):
    fd.write(
        'make new shape at end of graphics with properties {fill: no fill, draws stroke: false, draws shadow: false, autosizing: full, size: {57.000000, 30.000000}, text: {size: 16, alignment: center, font: "HelveticaNeue", text: "%s"}, origin: {100, 4.500000}}\n'
        % t
    )
