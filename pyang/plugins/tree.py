...
class TreePlugin(plugin.PyangPlugin):
    ...
    def add_opts(self, optparser):
        optlist = [
            ...
            optparse.make_option(
                "--tree-path", dest="tree_path", help="Subtree to print"
            ),
            optparse.make_option(
        ...
        if plugin.is_plugin_registered("restconf"):
            optlist.append(
                optparse.make_option(
                    ...
                    action="store_true",
                    help="Print ietf-restconf:yang-data " + "structures",
                )
            )
        if plugin.is_plugin_registered("structure"):
            optlist.append(
                optparse.make_option(
                    ...
                    action="store_true",
                    help="Print ietf-yang-structure-ext" + ":structure",
                )
            )
    ...
    def emit(self, ctx, modules, fd):
        if ctx.opts.tree_path is not None:
            ...
            path = None
        emit_tree(
            ctx, modules, fd, ctx.opts.tree_depth, ctx.opts.tree_line_length, path
        )
...
def emit_tree(ctx, modules, fd, depth, llen, path):
    ...
    for module in modules:
        ...
        for m in mods:
            for augment in m.search("augment"):
                if (
                    ...
                    and augment.i_target_node.i_module not in modules + mods
                ):
        ...
        if ctx.opts.tree_print_structures:
            ...
            sxs = module.search(("ietf-yang-structure-ext", "augment-structure"))
            if len(sxs) > 0:
                if not printed_header:
                    print_header(module)
                    printed_header = True
...
def print_children(
    ...
    for ch in i_children:
        if (ch.keyword == "input" or ch.keyword == "output") and len(
            ch.i_children
        ) == 0:
            pass
        else:
            if ch == i_children[-1] or (
                i_children[-1].keyword == "output"
...
def print_node(
    ...
    if s.keyword == "list":
        ...
        if s.keyword == "leaf-list":
            ...
            s.keyword == "leaf"
            and not hasattr(s, "i_is_key")
            or s.keyword == "anydata"
            or s.keyword == "anyxml"
        ):
            m = s.search_one("mandatory")
            if m is None or m.arg == "false":
        ...
        if t == "":
            ...
            llen is not None and len(line) + len(flags) + width + 1 + len(t) + 4 > llen
        ):
            # there's no room for the type name
            if get_leafref_path(s) is not None and len(t) + brcol > llen:
    ...
    if s.keyword == "list":
        if s.search_one("key") is not None:
            ...
            if llen is not None and len(line) + len(keystr) > llen:
                fd.write(line + "\n")
    ...
    if hasattr(s, "i_augment"):
        ...
        featurenames.extend([f.arg for f in afeatures if f.arg not in featurenames])
