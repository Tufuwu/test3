...
re_smi_oid = re.compile(
    r"^(([0-1](\.[1-3]?[0-9]))|(2\.(0|([1-9]\d*))))" r"(\.(0|([1-9]\d*)))*$"
)
...
def pyang_plugin_init():
    ...
    for stmt, occurence, (arg, rules), add_to_stmts in smi_stmts:
        ...
        grammar.add_to_stmts_rules(add_to_stmts, [((smi_module_name, stmt), occurence)])
    ...
    statements.add_validation_fun(
        "smi_set_oid", [(smi_module_name, "subid")], v_set_subid
    )

    # Register special error codes
    error.add_error_code(
        "SMIv2_BAD_SUBID", 1, "subid needs an oid or subid statement in an ancestor"
    )
    error.add_error_code(
        "SMIv2_SUBID_AND_OID", 1, "subid and oid cannot be given at the same time"
    )
...
smi_stmts = [
    ...
    ("display-hint", "?", ("string", []), ["leaf", "typedef"]),
    ("max-access", "?", ("smi-max-access", []), ["leaf", "typedef"]),
    ("defval", "?", ("string", []), ["leaf"]),
    ("implied", "?", ("identifier", []), ["list"]),
    (
        "alias",
        "*",
        (
