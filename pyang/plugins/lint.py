...
class LintPlugin(plugin.PyangPlugin):
    ...
    def add_opts(self, optparser):
        optlist = [
            optparse.make_option(
                ...
                dest="lint",
                action="store_true",
                help="Validate the module(s) according to " "RFC 8407rules.",
            ),
            optparse.make_option(
                "--lint-namespace-prefix",
    ...
    def _setup_ctx(self, ctx):
        ...
        statements.add_validation_var(
            "$chk_required", lambda keyword: keyword in _required_substatements
        )

        statements.add_validation_var(
            "$chk_recommended", lambda keyword: keyword in _recommended_substatements
        )

        statements.add_validation_fun(
            "grammar", ["$chk_default"], lambda ctx, s: v_chk_default(ctx, s)
        )
        statements.add_validation_fun(
            "grammar", ["$chk_required"], lambda ctx, s: v_chk_required_substmt(ctx, s)
        )
        statements.add_validation_fun(
            "grammar",
            ["$chk_recommended"],
        ...
        if self.ensure_hyphenated_names:
            statements.add_validation_fun(
                "grammar", ["*"], lambda ctx, s: v_chk_hyphenated_names(ctx, s)
            )
        ...
        statements.add_validation_fun(
            "strict", ["include"], lambda ctx, s: v_chk_include(ctx, s)
        )

        statements.add_validation_fun(
            "strict", ["module"], lambda ctx, s: v_chk_mandatory_top_level(ctx, s)
        )
        ...
        error.add_error_code(
            ...
            4,
            "RFC 8407: 4.4: " + 'statement "%s" is given with its default value "%s"',
        )
        error.add_error_code(
        ...
        error.add_error_code(
            ...
            4,
            "RFC 8407: 4.1: " + "the module name should start with the string %s",
        )
        error.add_error_code(
        ...
        error.add_error_code(
            ...
            4,
            "RFC 8407: 4.1: " + "no module name prefix string used",
        )
        error.add_error_code(
            "LINT_BAD_REVISION",
            3,
        ...
        error.add_error_code(
            "LINT_TOP_MANDATORY",
            3,
            "RFC 8407: 4.10: " + "top-level node %s must not be mandatory",
        )
        error.add_error_code(
            "LINT_NOT_HYPHENATED",
            4,
...
def v_chk_default(ctx, stmt):
    if (
        stmt.arg == _keyword_with_default[stmt.keyword]
        and stmt.parent.keyword != "refine"
    ):
        err_add(ctx.errors, stmt.pos, "LINT_EXPLICIT_DEFAULT", (stmt.keyword, stmt.arg))
...
def v_chk_module_name(ctx, stmt, modulename_prefixes):
    if modulename_prefixes:
        ...
        if len(modulename_prefixes) == 1:
            ...
            s = (
                ", ".join(['"' + p + '-"' for p in modulename_prefixes[:-1]])
                + ', or "'
                + modulename_prefixes[-1]
                + '-"'
            )
            err_add(ctx.errors, stmt.pos, "LINT_BAD_MODULENAME_PREFIX_N", s)
...
def v_chk_include(ctx, stmt):
    ...
    if (
        subm is not None
        and subm.i_latest_revision is not None
        and subm.i_latest_revision > latest
    ):
        err_add(
            ctx.errors,
            stmt.pos,
