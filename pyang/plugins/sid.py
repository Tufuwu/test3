...
class SidPlugin(plugin.PyangPlugin):
    ...
    def post_validate_ctx(self, ctx, modules):
        ...
        try:
            ...
            sys.stderr.write(
                "ERROR in '%s', invalid JSON content\n" % sid_file.input_file_name
            )
        else:
            sys.exit(0)
        sys.exit(1)
...
class SidFile:
    ...
    def process_sid_file(self, module):
        ...
        if self.input_file_name is not None:
            if not self.input_file_name.endswith(".sid"):
                raise SidParsingError(
                    "File '%s' is not a .sid file" % self.input_file_name
                )
        ...
        if self.extra_range == "count":
            ...
            print(
                "Number of SIDs required by this version: %d"
        ...
        if self.check_consistency:
            ...
            if self.is_consistent:
                print(
                    "No .sid file generated, the current .sid file is already up to date."
                )
            else:
                self.generate_file()
    ...
    def set_sid_range(self, srange):
        ...
        if not match:
            raise SidParsingError(
                "invalid range in argument, must be '<entry-point>:<size>'."
            )
            ...
            for key in arange:
                if key == "entry-point":
                    ...
                    if not isinstance(arange[key], util.int_types):
                        raise SidFileError(
                            "invalid 'entry-point' value '%s'." % arange[key]
                        )
        ...
        for item in items:
            for key in item:
                if key == "namespace":
                    ...
                    if not (
                        isinstance(item[key], util.str_types)
                        and item[key].endswith(self.namespace_ends)
                    ):
                        raise SidFileError(
                            "invalid 'namespace' value '%s'." % item[key]
                        )
                    ...
                    if not isinstance(item[key], util.str_types):
                        raise SidFileError(
                            "invalid 'identifier' value '%s'." % item[key]
                        )
    ...
    def collect_module_items(self, module):
        ...
        for item in self.content["items"]:
            item["status"] = (
                "d"  # Set to 'd' deleted, updated to 'o' if present in .yang file
            )
    ...
    def get_path(self, statement, prefix=""):
        ...
        while statement.i_module is not None:
            if (
                statement.keyword not in self.grouping_keywords
                ...
                if prefix != "" or (
                    parent.i_module is not None
                    and parent.i_module == statement.i_module
                ):
    ...
    def print_registration_information(self, module):
        ...
        for name in module.i_ctx.modules:
            ...
            if submodule.keyword == "submodule":
                submodules.append(
                    "%s@%s.yang" % (submodule.arg, submodule.i_latest_revision)
                )
