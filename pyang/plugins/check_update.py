...
class CheckUpdatePlugin(plugin.PyangPlugin):
    def add_opts(self, optparser):
        ...
        error.add_error_code(
            "CHK_INVALID_MODULENAME",
            1,
            "the module's name MUST NOT be changed" + " (RFC 6020: sec. 10, p3)",
        )
        error.add_error_code(
            "CHK_INVALID_MODULENAME_v1.1",
            1,
            "the module's name MUST NOT be changed" + " (RFC 7950: sec. 11, p3)",
        )
        error.add_error_code(
            "CHK_INVALID_NAMESPACE",
            1,
            "the module's namespace MUST NOT be changed" + " (RFC 6020: sec. 10, p3)",
        )
        error.add_error_code(
            "CHK_INVALID_NAMESPACE_v1.1",
            1,
            "the module's namespace MUST NOT be changed" + " (RFC 7950: sec. 11, p3)",
        )
        error.add_error_code(
            "CHK_NO_REVISION",
            1,
            "a revision statement MUST be present" + " (RFC 6020: sec. 10, p2)",
        )
        error.add_error_code(
            "CHK_NO_REVISION_v1.1",
            1,
            "a revision statement MUST be present" + " (RFC 7950: sec. 11, p2)",
        )
        error.add_error_code(
            "CHK_BAD_REVISION",
            1,
        ...
        error.add_error_code(
            "CHK_BAD_REVISION_v1.1",
            1,
            "new revision %s is not newer than old revision %s"
            + " (RFC 7950: sec. 11, p2)",
        )
        error.add_error_code(
            "CHK_DEF_REMOVED", 1, "the %s '%s', defined at %s is illegally removed"
        )
        error.add_error_code("CHK_DEF_ADDED", 1, "the %s '%s' is illegally added")
        error.add_error_code(
            "CHK_DEF_ADDED2", 1, "the %s '%s' is illegally added in %s %s"
        )
        error.add_error_code(
            "CHK_DEF_CHANGED", 1, "the %s '%s' is illegally changed from '%s'"
        )
        error.add_error_code(
            "CHK_INVALID_STATUS",
            1,
        ...
        error.add_error_code(
            "CHK_CHILD_KEYWORD_CHANGED", 1, "the %s '%s' is illegally changed to a %s"
        )
        error.add_error_code(
            "CHK_MANDATORY_CONFIG",
            1,
            "the node %s is changed to config true, but it is mandatory",
        )
        error.add_error_code(
            "CHK_NEW_MANDATORY", 1, "the mandatory node %s is illegally added"
        )
        error.add_error_code(
            "CHK_BAD_CONFIG", 1, "the node %s is changed to config false"
        )
        error.add_error_code("CHK_NEW_MUST", 1, "a new must expression cannot be added")
        error.add_error_code(
            "CHK_UNDECIDED_MUST",
            4,
            "this must expression may be more constrained than before",
        )
        error.add_error_code("CHK_NEW_WHEN", 1, "a new when expression cannot be added")
        error.add_error_code(
            "CHK_UNDECIDED_WHEN", 4, "this when expression may be different than before"
        )
        error.add_error_code(
            "CHK_UNDECIDED_PRESENCE",
            4,
            "this presence expression may be different than before",
        )
        error.add_error_code(
            "CHK_IMPLICIT_DEFAULT", 1, "the leaf had an implicit default"
        )
        error.add_error_code(
            "CHK_BASE_TYPE_CHANGED",
            1,
...
def check_update(ctx, newmod):
    ...
    for epos, etag, eargs in ctx.errors:
        if epos.ref in (newmod.pos.ref, oldmod.pos.ref) and error.is_error(
...
def chk_identity(olds, newmod, ctx):
    ...
    if newmod.i_version == "1.1":
        ...
        for old_id in set(old_ids) & set(new_ids):
            ...
            if (
                oldbase.i_identity.i_module.i_modulename
...
def chk_extension(olds, newmod, ctx):
    ...
    if oldarg is None and newarg is not None:
        ...
        elif oldyin is not None and newyin is not None and newyin.arg != oldyin.arg:
...
def chk_i_children(old, new, ctx):
    ...
    added_new_children = [
        new_child for new_child in new.i_children if new_child.arg not in old_child_args
    ]
...
def chk_default(old, new, ctx):
    ...
    if olddefault is not None and newdefault is None:
        ...
        if (
            oldtype.i_typedef is not None
            and hasattr(oldtype.i_typedef, "i_default_str")
            and oldtype.i_typedef.i_default is not None
            and oldtype.i_typedef.i_default_str != newdefault.arg
        ):
...
def chk_type(old, new, ctx):
    ...
    if oldts.name != newts.name:
        err_add(ctx.errors, new.pos, "CHK_BASE_TYPE_CHANGED", (oldts.name, newts.name))
        return
...
def chk_range(old, new, oldts, newts, ctx):
    ...
    if isinstance(ots, types.RangeTypeSpec):
        ...
        err_add(ctx.errors, nts.ranges_pos, "CHK_DEF_ADDED", ("range", str(nts.ranges)))
...
def chk_decimal64(old, new, oldts, newts, ctx):
    ...
    if newbasets.fraction_digits != oldbasets.fraction_digits:
        err_add(
            ...
            new.pos,
            "CHK_DEF_CHANGED",
            ("fraction-digits", newts.fraction_digits, oldts.fraction_digits),
        )
...
def chk_enumeration(old, new, oldts, newts, ctx):
    ...
    for name, val in oldts.enums:
        ...
        if n is None:
            err_add(ctx.errors, new.pos, "CHK_DEF_REMOVED", ("enum", name, old.pos))
        elif n[1] != val:
            errcode = verrcode("CHK_ENUM_VALUE_CHANGED", new)
            err_add(ctx.errors, new.pos, errcode, (name, val, n[1]))
...
def chk_bits(old, new, oldts, newts, ctx):
    ...
    for name, pos in oldts.bits:
        ...
        if n is None:
            err_add(ctx.errors, new.pos, "CHK_DEF_REMOVED", ("bit", name, old.pos))
        elif n[1] != pos:
            errcode = verrcode("CHK_BIT_POSITION_CHANGED", new)
            err_add(ctx.errors, new.pos, errcode, (name, pos, n[1]))
...
def chk_leafref(old, new, oldts, newts, ctx):
    ...
    if not hasattr(old.parent, "i_leafref_ptr") or not hasattr(
        new.parent, "i_leafref_ptr"
    ):
        return
    if old.parent.i_leafref_ptr is None or new.parent.i_leafref_ptr is None:
        return
    ...
    def cmp_node(optr, nptr):
        if optr.parent is None:
            return
        if (
            optr.i_module.i_modulename == nptr.i_module.i_modulename
            and optr.arg == nptr.arg
        ):
...
def chk_identityref(old, new, oldts, newts, ctx):
    ...
    for oidbase in oldts.idbases:
        for nidbase in newts.idbases:
            if (
                nidbase.i_module.i_modulename == oidbase.i_module.i_modulename
                and nidbase.arg.split(":")[-1] == oidbase.arg.split(":")[-1]
            ):
...
def err_def_removed(old, newp, ctx):
    ...
    err_add(ctx.errors, newp.pos, "CHK_DEF_REMOVED", (old.keyword, old_arg, old.pos))
