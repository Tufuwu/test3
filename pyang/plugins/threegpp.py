...
class THREEGPPlugin(lint.LintPlugin):
    ...
    def add_opts(self, optparser):
        optlist = [
            optparse.make_option(
                ...
                dest="threegpp",
                action="store_true",
                help="Validate the module(s) according to " "3GPP rules.",
            ),
        ]
    ...
    def setup_ctx(self, ctx):
        ...
        error.add_error_code(
            "3GPP_BAD_NAMESPACE_VALUE",
            3,
            "3GPP: the namespace should be urn:3gpp:sa5:%s",
        )

        statements.add_validation_fun(
            "grammar", ["namespace"], lambda ctx, s: self.v_chk_namespace(ctx, s)
        )

        error.add_error_code(
            "3GPP_BAD_PREFIX_VALUE", 3, "3GPP: the prefix should end with 3gpp"
        )

        error.add_error_code(
            "3GPP_TOO_LONG_PREFIX",
            3,
            "3GPP: the prefix should not be longer than 13 characters",
        )

        statements.add_validation_fun(
            "grammar", ["prefix"], lambda ctx, s: self.v_chk_prefix(ctx, s)
        )

        error.add_error_code(
            "3GPP_BAD_YANG_VERSION", 3, "3GPP: the yang-version should be 1.1"
        )

        statements.add_validation_fun(
            "grammar", ["yang-version"], lambda ctx, s: self.v_chk_yang_version(ctx, s)
        )
        ...
        statements.add_validation_fun(
            "grammar",
            ["anydata", "anyxml", "deviation", "rpc"],
            lambda ctx, s: self.v_chk_not_allowed_statements(ctx, s),
        )

        error.add_error_code(
            "3GPP_BAD_ORGANIZATION", 3, "3GPP: organization statement must include 3GPP"
        )

        statements.add_validation_fun(
            "grammar", ["organization"], lambda ctx, s: self.v_chk_organization(ctx, s)
        )

        error.add_error_code("3GPP_BAD_CONTACT", 3, "3GPP: incorrect contact statement")
        ...
        error.add_error_code(
            "3GPP_BAD_MODULE_REFERENCE",
            3,
            "3GPP: the module's reference substatement is incorrect",
        )

        statements.add_validation_fun(
            "grammar", ["reference"], lambda ctx, s: self.v_chk_module_reference(ctx, s)
        )
        ...
        error.add_error_code(
            "3GPP_LONG_LINE", 3, "3GPP: line longer than 80 characters"
        )
        ...
        error.add_error_code(
            "3GPP_NON_ASCII", 4, "3GPP: the module should only use ASCII characters"
        )

        statements.add_validation_fun(
            "grammar", ["module"], lambda ctx, s: self.v_chk_3gpp_format(ctx, s)
        )

        error.add_error_code(
            "3GPP_LIMITED_CONTAINER_USE",
            4,
            (
    ...
    def v_chk_namespace(self, ctx, stmt):
        ...
        if re.match(r, stmt.arg) is None:
            err_add(ctx.errors, stmt.pos, "3GPP_BAD_NAMESPACE_VALUE", stmt.i_module.arg)
    ...
    def post_validate_ctx(self, ctx, modules):
        ...
        for ctx_error in ctx.errors[:]:
            if (
                (
                    ctx_error[1] == "LINT_MISSING_REQUIRED_SUBSTMT"
