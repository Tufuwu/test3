...
top_stmts = [("$choice", [[("module", "1")], [("submodule", "1")]])]
...
stmt_map = {
    ...
    "belongs-to": ("identifier", [("prefix", "1")]),
    "organization": ("string", []),
    "contact": ("string", []),
    "description": ("string", []),
    "reference": ("string", []),
    ...
    "extension": (
        "identifier",
        [("argument", "?"), ("status", "?"), ("description", "?"), ("reference", "?")],
    ),
    "argument": ("identifier", [("yin-element", "?")]),
    "yin-element": ("boolean", []),
    "feature": (
        "identifier",
        [
    ...
    "input": (
        ...
        [
            ("$1.1", ("must", "*")),
            ("$interleave", [("typedef", "*"), ("grouping", "*")] + data_def_stmts),
        ],
    ),
    "output": (
        None,
...
def _chk_stmts(ctx, pos, stmts, parent, spec, canonical):
    for stmt in stmts:
        ...
        if match_res is None and chk_grammar:
            ...
            if arg_type is None and stmt.arg is not None:
                error.err_add(ctx.errors, stmt.pos, "UNEXPECTED_ARGUMENT", stmt.arg)
                ...
                and arg_type != "string"
                and syntax.arg_type_map[arg_type](stmt.arg) is False
            ):
                error.err_add(ctx.errors, stmt.pos, "BAD_VALUE", (stmt.arg, arg_type))
            elif (
                arg_type == "identifier"
                and ctx.max_identifier_len is not None
            ...
            if canonical:
                ...
                cansubspec = []
            _chk_stmts(
                ctx, stmt.pos, stmt.substmts, stmt, (subspec, cansubspec), canonical
            )
            spec = match_res
        else:
            # unknown extension
            stmt.is_grammatically_valid = True
            nspec = [("$any", "*")]
            _chk_stmts(ctx, stmt.pos, stmt.substmts, stmt, (nspec, nspec), canonical)
    ...
    for keywd, occurence in spec[0]:
        if occurence == "1" or occurence == "+":
            if parent is None:
                error.err_add(
                    ctx.errors, pos, "EXPECTED_KEYWORD", util.keyword_to_str(keywd)
                )
            else:
                error.err_add(
                    ctx.errors,
        ...
        if keywd == stmt.keyword:
            ...
            if occurence == "1" or occurence == "+":
                ...
                error.err_add(
                    ...
                    stmt.pos,
                    errcode,
                    (util.keyword_to_str(stmt.raw_keyword), util.keyword_to_str(keywd)),
                )
