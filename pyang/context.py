...
class Context(object):
    ...
    def add_parsed_module(self, module):
        ...
        if module.arg is None:
            error.err_add(self.errors, module.pos, "EXPECTED_ARGUMENT", module.keyword)
            return None
    ...
    def _ensure_revs(self, revs):
        ...
        while i < length:
            ...
            if rev is None:
                ...
                try:
                    ref, in_format, text = repository.get_module_from_handle(handle)
    ...
    def search_module(self, pos, modulename, revision=None, primary_module=False):
        ...
        if revision is not None:
            ...
            if x is not None:
                ...
                error.err_add(
                    self.errors, pos, "MODULE_NOT_FOUND_REV", (modulename, revision)
                )
                # keep track of this to avoid multiple errors
        ...
        if handle is None:
            ...
            try:
                ref, in_format, text = self.repository.get_module_from_handle(handle)
                module = self.add_module(
                    ref, text, in_format, modulename, revision, True, primary_module
                )
    ...
    def validate(self):
        ...
        for uri in uri_map:
            ...
            for pos in uses[0]:
                error.err_add(
                    self.errors, pos, "DUPLICATE_NAMESPACE", (uri, module_names)
                )
