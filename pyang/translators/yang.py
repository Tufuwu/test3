...
def emit_stmt(
    ...
    if (
        level == 1 and kwd_class != prev_kwd_class and kwd_class != "extension"
    ) and not (
        (level == 1 and prev_kwd in _keyword_with_trailing_blank_line_toplevel)
        or prev_kwd in _keyword_with_trailing_blank_line
    ):
        fd.write("\n")
    ...
    if stmt.arg is not None:
        ...
        if (
            stmt.keyword in _keyword_prefer_single_quote_arg
            and "'" not in stmt.arg
            and "\n" not in stmt.arg
        ):
            ...
            arg_on_new_line = emit_arg(
                keywordstr, stmt, fd, indent, indentstep, max_line_len, line_len
            )
        elif stmt.keyword in _keyword_with_path_arg:
            # special code for path argument; pretty-prints a long path with
            # line breaks
            arg_on_new_line = emit_path_arg(
                keywordstr, stmt.arg, fd, indent, max_line_len, line_len, eol
            )
            ...
            if arg_type in _non_quote_arg_type or (
                arg_type in _maybe_quote_arg_type and not need_quote(stmt.arg)
            ):
    ...
    if not islast and (
        (level == 1 and stmt.keyword in _keyword_with_trailing_blank_line_toplevel)
        or stmt.keyword in _keyword_with_trailing_blank_line
    ):
        fd.write("\n")
...
def emit_multi_str_arg(
    keywordstr, strs, fd, pref_q, indent, indentstep, max_line_len, line_len
):
...
def emit_path_arg(keywordstr, arg, fd, indent, max_line_len, line_len, eol):
    ...
    while arg != "":
        line_len = len("%s%s %s%s%s%s" % (indent, keyword_cont, quote, arg, quote, eol))
        if line_len <= max_line_len:
            fd.write("\n" + indent + keyword_cont + " " + quote + arg + quote)
            arg = ""
        else:
            # we need to split
            ...
            if arg[num_chars - 1 : num_chars] == "/":
                ...
                num_chars = len(arg) - (line_len - max_line_len)
            fd.write(
                "\n" + indent + keyword_cont + " " + quote + arg[:num_chars] + quote
            )
            arg = arg[num_chars:]
...
def emit_arg(keywordstr, stmt, fd, indent, indentstep, max_line_len, line_len):
    ...
    if len(lines) <= 1:
        if len(arg) > 0 and arg[-1] == "\n":
            arg = arg[:-1] + r"\n"
        if stmt.keyword in _force_newline_arg or need_new_line(
            max_line_len, line_len, arg
        ):
