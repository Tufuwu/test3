...
_deviate_delete_singleton_keywords = {"units": True, "default": True}
        ...
        if res == "continue":
            ...
            if phase in _v_i_children:
                ...
                for s in stmt.substmts:
                    if (
                        hasattr(s, "i_has_i_children")
                        or (phase, s.keyword) in _v_i_children_keywords
                    ):
                        iterate(s, phase)
            else:
                for s in stmt.substmts:
...
def v_init_module(ctx, stmt):
    ...
    for i in stmt.search("import"):
        ...
        if p is not None:
            ...
            if i.arg is not None and p.arg is not None and i.arg != stmt.i_modulename:
...
def v_grammar_unique_defs(ctx, stmt):
    ...
    """
    defs = [
        ("typedef", "TYPE_ALREADY_DEFINED", stmt.i_typedefs),
    ...
    for keyword, errcode, stmtdefs in defs:
        for definition in stmt.search(keyword):
            if definition.arg in stmtdefs:
                ...
                err_add(
                    ctx.errors, definition.pos, errcode, (definition.arg, other.pos)
                )
            else:
...
def v_grammar_identifier(ctx, stmt):
    ...
    if (
        arg_type == "identifier"
        and grammar.re_identifier_illegal_prefix.search(stmt.arg) is not None
    ):
...
def v_import_module(ctx, stmt):
    ...
    def add_module(i, primary_module):
        ...
        if m is not None and i.keyword == "import" and i.i_is_safe_import:
            ...
            err_add(ctx.errors, i.pos, "CIRCULAR_DEPENDENCY", ("module", modulename))
        # try to add the module to the context
        m = ctx.search_module(i.pos, modulename, rev, primary_module=primary_module)
        if m is not None:
            validate_module(ctx, m)
        if (
            m is not None
            and r is not None
            and stmt.i_version == "1"
            and m.i_version == "1.1"
        ):
            err_add(
                ctx.errors,
    ...
    for i in imports:
        ...
        if module is not None and module.keyword != "module":
            err_add(ctx.errors, i.pos, "BAD_IMPORT", (module.keyword, i.arg))

    for i in includes:
        submodule = add_module(i, stmt.i_is_primary_module)
        if submodule is not None and submodule.keyword != "submodule":
            err_add(ctx.errors, i.pos, "BAD_INCLUDE", (submodule.keyword, i.arg))
            return
        if submodule is not None:
            ...
            if b is not None and b.arg != mymodulename:
                ...
                defs = [
                    (submodule.i_typedefs, stmt.i_typedefs, "TYPE_ALREADY_DEFINED"),
                    (
                        submodule.i_groupings,
                        stmt.i_groupings,
                        "GROUPING_ALREADY_DEFINED",
                    ),
                    (submodule.i_features, stmt.i_features, "FEATURE_ALREADY_DEFINED"),
                    (
                        submodule.i_identities,
                        stmt.i_identities,
                        "IDENTITY_ALREADY_DEFINED",
                    ),
                    (
                        submodule.i_extensions,
                        stmt.i_extensions,
...
def v_type_typedef(ctx, stmt):
    if hasattr(stmt, "i_is_validated"):
        if stmt.i_is_validated is True:
            ...
            err_add(ctx.errors, stmt.pos, "CIRCULAR_DEPENDENCY", ("type", stmt.arg))
    ...
    if stmt.parent.parent is not None:
        ...
        if ptype is not None:
            err_add(ctx.errors, stmt.pos, "TYPE_ALREADY_DEFINED", (name, ptype.pos))
    ...
    def check_circular_typedef(ctx, type_):
        ...
        if (
            type_.i_typedef is not None
            and type_.i_typedef.is_grammatically_valid is True
        ):
    ...
    if (
        ...
        and type_.i_typedef.i_default is not None
    ):
        ...
        and type_.i_type_spec is not None
    ):
        stmt.i_default = type_.i_type_spec.str_to_val(
            ctx.errors, default.pos, default.arg, stmt.i_module
        )
...
def v_type_type(ctx, stmt):
    ...
    if prefix is None or stmt.i_module.i_prefix == prefix:
        ...
        if stmt.i_typedef is None:
            ...
            try:
                ...
                err_add(
                    ctx.errors, stmt.pos, "TYPE_NOT_FOUND", (name, stmt.i_module.arg)
                )
                return
        else:
        ...
        pmodule = util.prefix_to_module(stmt.i_module, prefix, stmt.pos, ctx.errors)
        if pmodule is None:
            return
    ...
    if stmt.i_typedef is not None:
        ...
        if typedef_type is not None and hasattr(typedef_type, "i_type_spec"):
            ...
            if stmt.i_type_spec is not None:
                stmt.i_type_spec.definition = "at " + str(stmt.i_typedef.pos) + " "
    ...
    if rangestmt is not None:
        if "range" not in stmt.i_type_spec.restrictions():
            ...
            if ranges_spec is not None:
                ...
                stmt.i_type_spec = types.RangeTypeSpec(stmt.i_type_spec, ranges_spec)

    # check the length restriction
    stmt.i_lengths = []
    length = stmt.search_one("length")
    if length is not None and "length" not in stmt.i_type_spec.restrictions():
        ...
        if lengths_spec is not None:
            ...
            stmt.i_type_spec = types.LengthTypeSpec(stmt.i_type_spec, lengths_spec)

    # check the pattern restrictions
    patterns = stmt.search("pattern")
    if patterns and "pattern" not in stmt.i_type_spec.restrictions():
        err_add(ctx.errors, patterns[0].pos, "BAD_RESTRICTION", "pattern")
    elif patterns:
        stmt.i_is_derived = True
        pattern_specs = [types.validate_pattern_expr(ctx.errors, p) for p in patterns]
        if None not in pattern_specs:
            # all patterns valid
            stmt.i_type_spec = types.PatternTypeSpec(stmt.i_type_spec, pattern_specs)
    ...
    if path is not None and stmt.arg != "leafref":
        ...
        err_add(ctx.errors, stmt.pos, "MISSING_TYPE_SPEC_1", ("leafref", "path"))
    elif path is not None:
        stmt.i_is_derived = True
        if path.is_grammatically_valid is True:
            path_spec = types.validate_path_expr(ctx.errors, path)
            if path_spec is not None:
                stmt.i_type_spec = types.PathTypeSpec(
                    stmt.i_type_spec, path_spec, path, path.pos
                )
    ...
    if bases and stmt.arg != "identityref":
        ...
        err_add(ctx.errors, stmt.pos, "MISSING_TYPE_SPEC", ("identityref", "base"))
    else:
        idbases = []
        for base in bases:
    ...
    if (
        req_inst is not None
        and "require-instance" not in stmt.i_type_spec.restrictions()
    ):
        err_add(ctx.errors, req_inst.pos, "BAD_RESTRICTION", "require-instance")
    if (
        req_inst is not None
        and stmt.i_type_spec.name == "leafref"
        and stmt.i_module.i_version == "1"
    ):
        err_add(ctx.errors, req_inst.pos, "BAD_RESTRICTION", "require-instance")
    ...
    if enums and (
        "enum" not in stmt.i_type_spec.restrictions()
        or stmt.i_module.i_version == "1"
        and stmt.arg != "enumeration"
    ):
        err_add(ctx.errors, enums[0].pos, "BAD_RESTRICTION", "enum")
    elif stmt.arg == "enumeration" and not enums:
        err_add(ctx.errors, stmt.pos, "MISSING_TYPE_SPEC", ("enumeration", "enum"))
    elif enums:
        stmt.i_is_derived = True

        enum_spec = types.validate_enums(ctx.errors, enums, stmt)
        if enum_spec is not None:
            stmt.i_type_spec = types.EnumTypeSpec(stmt.i_type_spec, enum_spec)
    ...
    if bits and (
        "bit" not in stmt.i_type_spec.restrictions()
        or stmt.i_module.i_version == "1"
        and stmt.arg != "bits"
    ):
        err_add(ctx.errors, bits[0].pos, "BAD_RESTRICTION", "bit")
    elif stmt.arg == "bits" and not bits:
        err_add(ctx.errors, stmt.pos, "MISSING_TYPE_SPEC", ("bits", "bit"))
    elif bits:
        stmt.i_is_derived = True
        bit_spec = types.validate_bits(ctx.errors, bits, stmt)
        if bit_spec is not None:
            stmt.i_type_spec = types.BitTypeSpec(stmt.i_type_spec, bit_spec)
    ...
    if membertypes and stmt.arg != "union":
        ...
        err_add(ctx.errors, stmt.pos, "MISSING_TYPE_SPEC", ("union", "type"))
    elif membertypes:
        stmt.i_is_derived = True
        for t in membertypes:
...
def v_type_leaf(ctx, stmt):
    ...
    if default is not None and type_.i_type_spec is not None:
        defval = type_.i_type_spec.str_to_val(
            ctx.errors, default.pos, default.arg, stmt.i_module
        )
        ...
        if defval is not None:
            type_.i_type_spec.validate(
                ctx.errors, default.pos, defval, stmt.i_module, " for the default value"
            )
    elif (
        default is None
        and type_.i_typedef is not None
...
def v_type_leaf_list(ctx, stmt):
    ...
    if (
        ...
        and min_value.arg.isnumeric()
        and int(min_value.arg) > 0
    ):
...
def v_type_grouping(ctx, stmt):
    if hasattr(stmt, "i_is_validated"):
        if stmt.i_is_validated is True:
            ...
            err_add(ctx.errors, stmt.pos, "CIRCULAR_DEPENDENCY", ("grouping", stmt.arg))
            return False
    ...
    if stmt.parent.parent is not None:
        ...
        if pgrouping is not None:
            err_add(
                ctx.errors, stmt.pos, "GROUPING_ALREADY_DEFINED", (name, pgrouping.pos)
            )

    # search for circular grouping definitions
    def validate_uses(s):
        if s.keyword == "uses" and getattr(s, "is_grammatically_valid", None) is True:
    ...
    if hasattr(stmt, "i_grouping"):
        if stmt.i_grouping is None and no_error_report is False:
            if prefix is None or stmt.i_module.i_prefix == prefix:
                ...
                if pmodule is None:
                    return
            err_add(ctx.errors, stmt.pos, "GROUPING_NOT_FOUND", (name, pmodule.arg))
        return
...
def v_type_augment(ctx, stmt):
    ...
    if stmt.parent.keyword == "uses" and stmt.arg.startswith("/"):
        ...
        err_add(ctx.errors, stmt.pos, "BAD_VALUE", (stmt.arg, "descendant-node-id"))
    elif stmt.parent.keyword != "uses" and not stmt.arg.startswith("/"):
        stmt.i_target_node = None
        err_add(ctx.errors, stmt.pos, "BAD_VALUE", (stmt.arg, "absolute-node-id"))
...
def v_type_extension(ctx, stmt):
    ...
    if identifier not in module.i_extensions:
        if module.i_modulename == stmt.i_orig_module.i_modulename:
            ...
            err_add(
                ctx.errors, stmt.pos, "EXTENSION_NOT_DEFINED", (identifier, module.arg)
            )
            return
    else:
    ...
    if stmt.arg is not None and ext_arg is None:
        err_add(ctx.errors, stmt.pos, "EXTENSION_ARGUMENT_PRESENT", identifier)
    elif stmt.arg is None and ext_arg is not None:
        err_add(ctx.errors, stmt.pos, "EXTENSION_NO_ARGUMENT_PRESENT", identifier)
...
def v_type_feature(ctx, stmt):
    if hasattr(stmt, "i_is_validated"):
        if stmt.i_is_validated is True:
            ...
            err_add(ctx.errors, stmt.pos, "CIRCULAR_DEPENDENCY", ("feature", stmt.arg))
            return
...
def v_type_if_feature(ctx, stmt, no_error_report=False):
    ...
    if stmt.i_module.i_version == "1":
        ...
        if not isinstance(expr, util.str_types):
            err_add(ctx.errors, stmt.pos, "BAD_VALUE", (stmt.arg, "identifier-ref"))
            return
    ...
    def has_feature(name):
        ...
        if found is None and no_error_report is False:
            err_add(ctx.errors, stmt.pos, "FEATURE_NOT_FOUND", (name, pmodule.arg))
            raise Abort
...
def v_type_identity(ctx, stmt):
    if hasattr(stmt, "i_is_validated"):
        if stmt.i_is_validated is True:
            ...
            err_add(ctx.errors, stmt.pos, "CIRCULAR_DEPENDENCY", ("identity", stmt.arg))
            return
...
def v_expand_1_children(ctx, stmt):
    ...
    for s in stmt.substmts:
        ...
        elif s.keyword == "uses" and getattr(s, "is_grammatically_valid", None):
...
def v_default(ctx, target, default):
    ...
    if type_ is not None and getattr(type_, "i_type_spec", None) is not None:

        defval = type_.i_type_spec.str_to_val(
            ctx.errors, default.pos, default.arg, target.i_module
        )
...
def v_expand_1_uses(ctx, stmt):
    ...
    def find_refine_node(refinement):
        ...
        for prefix, identifier in path:
            ...
            if hasattr(node, "i_children"):
                if module is None:
                    return None
                child = search_child(node.i_children, module.i_modulename, identifier)
                if child is None:
                    err_add(
                        ctx.errors,
                        refinement.pos,
    ...
    def replace_from_refinement(
        target, refinement, keyword, valid_keywords, v_fun=None
    ):
        ...
        if new is not None and target.keyword in valid_keywords:
            ...
            err_add(
                ...
                refinement.pos,
                "BAD_REFINEMENT",
                (target.keyword, target.i_module.i_modulename, target.arg, keyword),
            )
            return

    def merge_from_refinement(target, refinement, keyword, valid_keywords, v_fun=None):
        """allow `keyword` as a refinement in `valid_keywords`"""
    ...
    for g in stmt.i_grouping.i_children:
        ...
        newg = g.copy(
            stmt.parent,
            stmt,
            nocopy=["type", "uses", "unique", "if-feature", "typedef", "grouping"],
            copyf=post_copy,
        )
        for s in whens:
            news = s.copy(newg)
    ...
    for refinement in stmt.search("refine"):
        ...
        for keyword, valid_keywords0, merge, v_fun in _refinements:
            ...
            if merge:
                merge_from_refinement(
                    target, refinement, keyword, valid_keywords, v_fun
                )
            else:
                replace_from_refinement(
                    target, refinement, keyword, valid_keywords, v_fun
                )
...
def v_expand_2_augment(ctx, stmt):
    ...
    if not hasattr(stmt.i_target_node, "i_children"):
        err_add(
            ...
            "BAD_NODE_IN_AUGMENT",
            (
                stmt.i_target_node.i_module.arg,
                stmt.i_target_node.arg,
    ...
    def add_tmp_children(node, tmp_children):
        for tmp in tmp_children:
            ch = search_child(node.i_children, stmt.i_module.i_modulename, tmp.arg)
            if ch is not None:
                ...
                if not hasattr(ch, "i_children"):
                    err_add(
                        ...
                        tmp.pos,
                        "BAD_NODE_IN_AUGMENT",
                        (stmt.i_module.i_modulename, ch.arg, ch.keyword),
                    )
                    raise Abort
    ...
    for c in stmt.i_children:
        ...
        ch = search_child(
            stmt.i_target_node.i_children, stmt.i_module.i_modulename, c.arg
        )
        if ch is not None:
            if ch.keyword == "__tmp_augment__":
                ...
                if not hasattr(c, "i_children"):
                    err_add(
                        ...
                        stmt.pos,
                        "BAD_NODE_IN_AUGMENT",
                        (stmt.i_module.i_modulename, c.arg, c.keyword),
                    )
                    return
    ...
    def f(s):
        for keyword, errcode, stmtdefs in defs:
            if s.keyword == keyword and s.arg in stmtdefs:
                err_add(ctx.errors, stmtdefs[s.arg].pos, errcode, (s.arg, s.pos))
            ...
            for x in key.arg.split():
                ...
                if stmt.i_module.i_version == "1":
                    if type_ is not None:
                        ...
                        if t is not None:
                            err_add(ctx.errors, key.pos, "BAD_TYPE_IN_KEY", (t.arg, x))
                            return
                ...
                if mandatory is not None and mandatory.arg == "false":
                    err_add(ctx.errors, mandatory.pos, "KEY_HAS_MANDATORY_FALSE", ())

                if ptr.i_config != stmt.i_config:
                    err_add(
                        ctx.errors, ptr.search_one("config").pos, "KEY_BAD_CONFIG", name
                    )
        ...
        for u in uniques:
            ...
            for expr in u.arg.split():
                ...
                for x in expr.split("/"):
                    if x == "":
                        continue
                    if ptr.keyword not in ["container", "list", "choice", "case"]:
            ...
            def chk_no_defaults(s):
                for c in s.i_children:
                    if c.keyword in ("leaf", "anydata", "anyxml", "choice"):
                        ...
                        if m is not None and m.arg == "true":
                            err_add(
                                ctx.errors, c.pos, "MANDATORY_NODE_IN_DEFAULT_CASE", ()
                            )
...
def v_reference_leaf_leafref(ctx, stmt):
    ...
    if (
        ...
        if ptr is not None:
            ...
            if not hasattr(stmt, "i_not_implemented") and hasattr(
                ptr, "i_not_implemented"
            ):
                err_add(ctx.errors, stmt.pos, "LEAFREF_TO_NOT_IMPLEMENTED", ())
...
def v_reference_deviate(ctx, stmt):
    ...
    if stmt.arg == "not-supported":
        ...
        del siblings[idx]
        if len(siblings) > 0:
            err_add(ctx.errors, siblings[0].pos, "BAD_DEVIATE_WITH_NOT_SUPPORTED", ())
            return
        if t.parent.keyword == "list" and t in t.parent.i_key:
            err_add(ctx.errors, stmt.pos, "BAD_DEVIATE_KEY", (t.i_module.arg, t.arg))
            return
        ...
        for c in stmt.substmts:
            ...
            if c.keyword == "config" and hasattr(t, "i_config"):
                ...
                if t.search_one(c.keyword) is not None:
                    ...
                    err_add(ctx.errors, c.pos, "BAD_DEVIATE_TYPE", c.keyword)
                else:
                    t.substmts.append(c)
            else:
                ...
                if c.keyword not in _valid_deviations:
                    if util.is_prefixed(c.keyword):
                        ...
                        pmodule = util.prefix_to_module(c.i_module, prefix, c.pos, [])
                        if (
                            pmodule is not None
                            and pmodule.modulename in grammar.extension_modules
                        ):
                            err_add(ctx.errors, c.pos, "BAD_DEVIATE_TYPE", c.keyword)
        ...
        for c in stmt.substmts:
            ...
            if c.keyword == "config" and hasattr(t, "i_config"):
                ...
                if c.arg == "true":
                    if negc.arg != c.arg:
                        if t.parent is not None and t.parent.i_config is False:
...
def v_unused_module(ctx, module):
    for prefix in module.i_unused_prefixes:
        ...
        err_add(ctx.errors, import_.pos, "UNUSED_IMPORT", import_.arg)

    for s in module.i_undefined_augment_nodes:
        err_add(ctx.errors, s.pos, "NODE_NOT_FOUND", (s.i_module.arg, s.arg))
...
def v_unused_typedef(ctx, stmt):
    if stmt.parent.parent is not None:
        ...
        if stmt.i_is_unused is True:
            err_add(ctx.errors, stmt.pos, "UNUSED_TYPEDEF", stmt.arg)
...
def v_unused_grouping(ctx, stmt):
    if stmt.parent.parent is not None:
        ...
        if stmt.i_is_unused is True:
            err_add(ctx.errors, stmt.pos, "UNUSED_GROUPING", stmt.arg)
    ...
    for child in children:
        if child.arg == identifier:
            if (
                ...
                or child.i_module.i_including_modulename is not None
                and child.i_module.i_including_modulename == modulename
            ):
                return child
    return None
...
def search_typedef(stmt, name):
    ...
    while stmt is not None:
        if name in stmt.i_typedefs:
            ...
            if (
                mod is not None
                and mod != t.i_orig_module
                and t.i_orig_module.keyword == "submodule"
            ):
...
def search_grouping(stmt, name):
    ...
    while stmt is not None:
        if name in stmt.i_groupings:
            ...
            if (
                mod is not None
                and mod != g.i_orig_module
                and g.i_orig_module.keyword == "submodule"
            ):
                # make sure this submodule is included
...
def search_data_keyword_child(children, modulename, identifier):
    for child in children:
        if (
            child.arg == identifier
            and child.i_module.i_modulename == modulename
            and child.keyword in data_keywords
        ):
            return child
    return None
...
def find_target_node(ctx, stmt, is_augment=False):
    ...
    if stmt.parent.keyword in ("module", "submodule") or is_absolute:
        ...
        chs = [
            c
            for c in stmt.parent.parent.i_children
    ...
    for prefix, identifier in path[1:]:
        if hasattr(node, "i_children"):
            ...
            if child is None and module == stmt.i_module and is_augment:
                ...
                child = Statement(
                    node.top, node, stmt.pos, "__tmp_augment__", identifier
                )
...
def is_submodule_included(src, tgt):
    ...
    if tgt is None or not hasattr(tgt, "i_orig_module"):
        return True
    if (
        tgt.i_orig_module.keyword == "submodule"
        and src.i_orig_module != tgt.i_orig_module
        and src.i_orig_module.i_modulename == tgt.i_orig_module.i_modulename
    ):
        if (
            src.i_orig_module.keyword == "submodule"
            and src.i_orig_module.i_version != "1"
        ):
            # In 1.1, if both src and tgt are submodules, src doesn't
            # have to include tgt
            return True
        if src.i_orig_module.search_one("include", tgt.i_orig_module.arg) is None:
            return False
    return True


def validate_leafref_path(
    ctx,
    stmt,
    path_spec,
    path,
    ...
    # grouping is used.
    if path.parent.parent is not None and path.parent.parent.keyword == "typedef":
    ...
    def find_identifier(identifier):
        if util.is_prefixed(identifier):
            ...
            if (
                path.i_module.keyword == "submodule"
                and prefix == local_module.i_prefix
                and local_module is not None
            ):
                pmodule = util.prefix_to_module(
    ...
    def follow_path(ptr, up, dn):
        ...
        if up == -1:  # absolute path
            ...
            if ptr is None:
                ...
                for inc in path.i_orig_module.search("include"):
                    ...
                    if submod is not None:
                        ptr = search_child(submod.i_children, submod.arg, name)
                        if ptr is not None:
                            break
                if ptr is None:
                    err_add(
            ...
            while up > 0:
                if ptr is None:
                    err_add(
                        ctx.errors, pathpos, "LEAFREF_TOO_MANY_UP", (stmt.arg, stmt.pos)
                    )
                    raise NotFound
        ...
        while i < len(dn):
            ...
            if ptr.keyword in _keyword_with_children:
                ptr = search_data_node(ptr.i_children, module_name, name, last_skipped)
                if not is_submodule_included(path, ptr):
                    ptr = None
                if ptr is None:
                    err_add(
    ...
    try:
        ...
        if derefup > 0:
            ...
            if ptr.keyword != "leaf":
                err_add(
                    ctx.errors, pathpos, "LEAFREF_DEREF_NOT_LEAFREF", (ptr.arg, ptr.pos)
                )
                return None
            ...
            if not hasattr(derefed_stmt, "i_is_key"):
                ...
                err_add(
                    ...
                    (ptr.arg, ptr.pos, derefed_stmt.arg, derefed_stmt.pos),
                )
                return None
            ...
            d1 = m.group(1)
            d2 = m.group(2)
            expanded_path = "%s[%s = current()/%s]/%s" % (s1, s2, d1, d2)
        ...
        if ptr.keyword not in ("leaf", "leaf-list") and not accept_non_leaf_target:
            err_add(ctx.errors, pathpos, "LEAFREF_NOT_LEAF", (stmt.arg, stmt.pos))
            return None
        if key_list == ptr.parent and (ptr.i_module.i_modulename, ptr.arg) in keys:
            err_add(
                ctx.errors,
                pathpos,
                "LEAFREF_MULTIPLE_KEYS",
        ...
        if (
            getattr(stmt, "i_config", None) is True
            and getattr(ptr, "i_config", None) is False
            and not accept_non_config_target
        ):
            err_add(
                ctx.errors, pathpos, "LEAFREF_BAD_CONFIG", (stmt.arg, ptr.arg, ptr.pos)
            )
        if ptr == stmt:
            err_add(ctx.errors, pathpos, "CIRCULAR_DEPENDENCY", ("leafref", path.arg))
            return None
...
class Statement(object):
    ...
    def copy(
        self, parent=None, uses=None, uses_top=True, nocopy=(), ignore=(), copyf=None
    ):
        new = copy.copy(self)
        ...
        for s in self.substmts:
            if s.keyword in ignore:
                ...
                new.substmts.append(s.copy(new, uses, False, nocopy, ignore, copyf))
    ...
    def main_module(self):
        ...
        if self.i_module.keyword == "submodule":
            return self.i_module.i_ctx.get_module(self.i_module.i_including_modulename)
...
class MustStatement(Statement):
    __slots__ = ("i_xpath",)  # parsed xpath expression | None
