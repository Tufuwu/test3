    ...
    for table in all_tables:
        ...
        for found_tag in table_tags["Tags"]:
            if found_tag["Key"] == tag_key:
                logging.debug(
                    "Checking table " + table + " tag " + found_tag["Key"]
                )
...
def get_restore_table_matches(table_name_wildcard, separator):
    ...
    try:
        ...
        try:
            ...
            logging.info(
                'Cannot find "%s" directory containing dump files!'
                % dump_data_path
            )
            sys.exit(1)
    ...
    for dir_name in dir_list:
        if table_name_wildcard == "*":
            ...
            dir_name.split(separator, 1)[0]
            == table_name_wildcard.split("*", 1)[0]
        ):
            matching_tables.append(dir_name)
...
def change_prefix(
    source_table_name, source_wildcard, destination_wildcard, separator
):
    """
    ...
    if separator == "":
        if (
            re.sub(r"([A-Z])", r" \1", source_table_name).split()[0]
            == source_prefix
        ):
    ...
    if source_table_name.split(separator, 1)[0] == source_prefix:
        return (
            destination_prefix
            + separator
            + source_table_name.split(separator, 1)[1]
        )
        ...
        while True:
            ...
            try:
                ...
                if (
                    ...
                    logging.info(
                        "Limit exceeded, retrying deletion of "
                        + table_name
                        + ".."
                    )
            ...
            try:
                while True:
                    logging.info(
                        ...
                        + table_name
                        + " table to be deleted.. ["
                        + conn.describe_table(table_name)["Table"][
                            "TableStatus"
                        ]
                        + "]"
                    )
...
def do_empty(dynamo, table_name):
    ...
    original_read_capacity = table_desc["ProvisionedThroughput"][
        "ReadCapacityUnits"
    ]
    original_write_capacity = table_desc["ProvisionedThroughput"][
        "WriteCapacityUnits"
    ]
    ...
    while True:
        try:
            ...
            if (
                ...
                logging.info(
                    "Limit exceeded, retrying creation of " + table_name + ".."
                )
                time.sleep(sleep_interval)
            elif (
                e.body["__type"]
...
def do_backup(dynamo, read_capacity, tableQueue=None, srcTable=None):
    ...
    if tableQueue:
        while True:
            ...
            f = open(
                args.dumpPath + os.sep + table_name + os.sep + SCHEMA_FILE,
                "w+",
            )
            ...
            if not args.schemaOnly:
                original_read_capacity = table_desc["Table"][
                    "ProvisionedThroughput"
                ]["ReadCapacityUnits"]
                original_write_capacity = table_desc["Table"][
                    "ProvisionedThroughput"
                ]["WriteCapacityUnits"]
                ...
                mkdir_p(
                    args.dumpPath + os.sep + table_name + os.sep + DATA_DIR
                )
                ...
                while True:
                    try:
                        scanned_table = dynamo.scan(
                            table_name, exclusive_start_key=last_evaluated_key
                        )
...
def do_restore(
    dynamo, sleep_interval, source_table, destination_table, write_capacity
):
    """
    Restore table
    """
    ...
    original_read_capacity = table["ProvisionedThroughput"][
        "ReadCapacityUnits"
    ]
    original_write_capacity = table["ProvisionedThroughput"][
        "WriteCapacityUnits"
    ]
    ...
    if table_global_secondary_indexes is not None:
        for gsi in table_global_secondary_indexes:
            ...
            if gsi["ProvisionedThroughput"]["WriteCapacityUnits"] < int(
                write_capacity
            ):
                gsi["ProvisionedThroughput"]["WriteCapacityUnits"] = int(
                    write_capacity
                )
        ...
        while True:
            try:
                ...
                if (
                    ...
                    logging.info(
                        "Limit exceeded, retrying creation of "
                        + destination_table
                        + ".."
                    )
                    time.sleep(sleep_interval)
    ...
    if not args.schemaOnly:
        ...
        for data_file in data_file_list:
            logging.info(
                "Processing " + data_file + " of " + destination_table
            )
            items = []
            item_data = json.load(
                open(
            ...
            if table_global_secondary_indexes is not None:
                ...
                for gsi in table_global_secondary_indexes:
                    ...
                    original_gsi_write_capacity = (
                        original_gsi_write_capacities.pop(0)
                    )
...
def main():
    ...
    parser.add_argument(
        "-a",
        "--archive",
        help="Type of compressed archive to create."
        "If unset, don't create archive",
        choices=["zip", "tar"],
    )
    parser.add_argument(
        "-b",
        "--bucket",
        help="S3 bucket in which to store or retrieve backups."
        "[must already exist]",
    )
    parser.add_argument(
        "-m",
    ...
    if args.mode == "backup":
        ...
        try:
            ...
            try:
                logging.info(
                    "Backup of table(s) " + args.srcTable + " completed!"
                )
            ...
            if args.archive:
                if args.tag:
                    for table in matching_backup_tables:
                        ...
                        did_archive, archive_file = do_archive(
                            args.archive, dump_path
                        )
