...
def emit_stmt(stmt, fd, ind, attrs="", n=None):
    if n is None and stmt.keyword in ["list", "leaf-list"]:
        ...
        ub = 3  # 3 is many
        if (
            stmt.search_one("max_elements") is not None
            and stmt.search_one("max_elements").arg != "unbounded"
        ):
...
def emit_leaf(stmt, fd, ind, attrs):
    do_print = True
    if (
        (stmt.search_one("mandatory") is not None)
        and (stmt.search_one("mandatory").arg == "false")
        or (stmt.search_one("default") is not None)
        or (statements.has_type(stmt.search_one("type"), ["empty"]) is not None)
    ):
        if random() < 0.3:
            do_print = False
    if do_print:
