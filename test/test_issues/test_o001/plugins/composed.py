...
class ComposedPlugin(plugin.PyangPlugin):
    def setup_ctx(self, ctx):
        ...
        statements.add_validation_fun("grammar", ["submodule"], v_chk_submodule)

        statements.add_validation_fun("grammar", ["module", "submodule"], v_chk_both)

        error.add_error_code(
            "COMPOSE_MODULE_ONLY", 2, "Module-only validation called for %s %s"
        )

        error.add_error_code(
            "COMPOSE_SUBMODULE_ONLY", 2, "Submodule-only validation called for %s %s"
        )
...
def v_chk_submodule(ctx, stmt):
    err_add(
        ctx.errors, stmt.pos, "COMPOSE_CALLED", ("submodule", stmt.keyword, stmt.arg)
    )
    if stmt.keyword != "submodule":
        err_add(
            ctx.errors, stmt.pos, "COMPOSE_SUBMODULE_ONLY", (stmt.keyword, stmt.arg)
        )
