...
try:
    ...
    print(
        "Pyserial is not installed for %s. Check the README for installation instructions."
        % (sys.executable)
    )
    raise
...
try:
    ...
    print(
        "The installed version (%s) of pyserial appears to be too old for esptool.py (Python interpreter %s). "
...
def stub_and_esp32_function_only(func):
    ...
    return check_supported_function(
        func, lambda o: o.IS_STUB or isinstance(o, ESP32ROM)
    )
...
class ESPLoader(object):
    ...
    def detect_chip(
        ...
        try:
            ...
            raise FatalError(
                "Unsupported Command Error received. Probably this means Secure Download Mode is enabled, "
    ...
    def connect(
        self, mode="default_reset", attempts=DEFAULT_CONNECT_ATTEMPTS, detecting=False
    ):
        ...
        if last_error is not None:
            raise FatalError(
                "Failed to connect to %s: %s" % (self.CHIP_NAME, last_error)
            )
            ...
            try:
                ...
                if chip_magic_value != self.CHIP_DETECT_MAGIC_VALUE:
                    ...
                    if actually is None:
                        print(
                            (
                                "WARNING: This chip doesn't appear to be a %s (chip magic value 0x%08x). "
    ...
    def flash_begin(self, size, offset, begin_rom_encrypted=False):
        ...
        if (
            isinstance(self, (ESP32S2ROM, ESP32S3BETA2ROM, ESP32C3ROM))
...
class ESP32S2ROM(ESP32ROM):
    ...
    def _check_if_can_reset(self):
        ...
        if (
            ...
            print(
                "ERROR: {} chip was placed into download mode using GPIO0.\n"
                "esptool.py can not exit the download mode over USB. "
                "To run the app, reset the chip manually.\n"
                "To suppress this error, set --after option to 'no_reset'.".format(
...
class ELFFile(object):
    ...
    def _read_segments(self, f, segment_header_offs, segment_header_count, shstrndx):
        ...
        def read_segment_header(offs):
            seg_type, seg_offs, _vaddr, lma, size, _memsize, _flags, _align = (
...
def detect_flash_size(esp, args):
    if args.flash_size == "detect":
        ...
        if args.flash_size is None:
            print(
                "Warning: Could not auto-detect Flash size (FlashID=0x%x, SizeID=0x%x), defaulting to 4MB"
                % (flash_id, size_id)
            )
            args.flash_size = "4MB"
        else:
...
def _update_image_flash_params(esp, address, args, image):
    ...
    if magic != esp.ESP_IMAGE_MAGIC:
        print(
            "Warning: Image file at 0x%x doesn't look like an image file, so not changing any flash settings."
            % address
        )
        return image
    ...
    try:
        ...
        print(
            "Warning: Image file at 0x%x is not a valid %s image, so not changing any flash settings."
    ...
    if args.encrypt or args.encrypt_files is not None:
        ...
        if not esp.secure_download_mode:
            if esp.get_encrypted_download_disabled():
                raise FatalError(
                    "This chip has encrypt functionality in UART download mode disabled. "
...
def main(argv=None, esp=None):
    ...
    parser = argparse.ArgumentParser(
        description="esptool.py v%s - ESP8266 ROM Bootloader Utility" % __version__,
        prog="esptool",
    )
    ...
    def add_spi_flash_subparsers(parent, allow_keep, auto_detect):
        ...
        parent.add_argument(
            "--flash_size",
            "-fs",
            help="SPI Flash size in MegaBytes (1MB, 2MB, 4MB, 8MB, 16M)"
    ...
    parser_elf2image.add_argument(
        "--output",
        "-o",
        help="Output filename prefix (for version 1 image), or filename (for version 2 single image)",
        type=str,
    )
    parser_elf2image.add_argument(
    ...
    parser_elf2image.add_argument(
        "--elf-sha256-offset",
        help="If set, insert SHA256 hash (32 bytes) of the input ELF file at specified offset in the binary.",
        type=arg_auto_int,
        default=None,
    )
    parser_elf2image.add_argument(
        "--use_segments",
        help="If set, ELF segments will be used instead of ELF sections to genereate the image.",
        action="store_true",
    )
    ...
    if (
        ...
        raise FatalError(
            "Options --encrypt and --encrypt-files must not be specified at the same time."
        )
    ...
    if (
        ...
        if esp is None:
            raise FatalError(
                "Could not connect to an Espressif device on any of the %d available serial ports."
                % len(ser_list)
            )
...
def get_port_list():
    if list_ports is None:
        raise FatalError(
            "Listing all serial ports is currently not available. Please try to specify the port when "
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        if value.upper() == "SPI":
            ...
            if len(values) != 5:
                raise argparse.ArgumentError(
                    self,
                    "%s is not a valid list of comma-separate pin numbers. Must be 5 numbers - CLK,Q,D,HD,CS."
                    % value,
                )
            try:
...
ESP32S3BETA2ROM.STUB_CODE = eval(
    zlib.decompress(
        base64.b64decode(
            b"""
